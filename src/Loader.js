...
class SubloaderComponent extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    setInterval(
      (window.location = this.props.location.state.redirectPath),
      10000
    );
  }
  setTimer(time) {
    ...
    return (
      <div className="lwrapper">
        <div style={styles} className={className}>
          ...
          <p>
            {this.props.location.state.redirectToTitle
              ? this.props.location.state.redirectToTitle
              : ""}
          </p>
        </div>
...
const Loader = ({ component: Component, ...rest }) => {
  ...
  return <SubloaderComponent data={data} {...rest} />;
};
